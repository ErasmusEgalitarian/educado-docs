...
The database is a bit weird with how it handles files that don't exist. Instead of returning Error 404, it returns a text object describing the error. For that reason, we check the HEAD to see if it returns `video/`, anything else will trigger an Error.
