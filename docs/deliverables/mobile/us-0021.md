...
- `LectureScreen.tsx` is unneeded. `lectureType` should determine the type of the lesson, and then the `ComponentSwipeScreen.tsx` screen should render `VideoLectureScreen.tsx`, not the `LectureScreen.tsx`, which in turn renders the `VideoLectureScreen.tsx` component.
